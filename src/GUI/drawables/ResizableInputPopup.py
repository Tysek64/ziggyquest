...
class ResizableInputPopup(ResizeMixin, CompositeMixin):
    def __init__(self, position: pygame.Rect, text: str, color: pygame.Color,
        ...
        self.color = color
        self.__top_margin = 20
        self.__left_margin = 20
        self.button_size = (self.position.width / 3, self.position.height / 6) # wybrane dowolnie, zeby ladnie wygladalo
        ...
        cancel_pos = (self.position.bottomleft[0] + self.__left_margin, self.text.get_rect().bottomleft[1] + self.__top_margin)
        self.cancel_button = ResizableButton(pygame.Rect(cancel_pos, self.button_size), 'Cancel',
                                             pygame.Color(255, 128, 0), on_cancel, parent_surface)

        accept_pos = (
        self.position.bottomright[0] - self.__left_margin - self.button_size[0], self.text.get_rect().bottomleft[1] + self.__top_margin)
        self.accept_button = ResizableButton(pygame.Rect(accept_pos, self.button_size), 'Confirm',
                                             pygame.Color(128, 255, 0), on_accept, parent_surface)

        self._rendered_objects = [self.accept_button, self.cancel_button]
        self.points = [self.accept_button.position.topleft, self.cancel_button.position.topleft]
